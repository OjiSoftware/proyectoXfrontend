...
import { useSubCategoryEdit } from "@/hooks/useSubCategoryEdit";
import { SquarePen } from "lucide-react";

export default function EditSubCategoryPage() {
    const navigate = useNavigate();
    const { id, name, setName, categoryId, setCategoryId, categories, loading, handleSubmit } = useSubCategoryEdit();

    const [showConfirmModal, setShowConfirmModal] = useState(false);

    if (!id) return null;

    const isFormInvalid = !name.trim() || categoryId === 0 || loading;
                ...
                <div className="flex justify-between items-end mb-4">
                    <div>
                        ...
                        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                            <SquarePen className="text-pink-400" size={24} />
                            Editar subcategoría
                        </h1>
                    </div>
                    <p className="text-slate-400 text-xs hidden md:block">
                        Modifique la informaci�n de la subcategoría.
                    </p>
                </div>
